...
using namespace std;
//NOTE: Obratiti paznju na kolicinu memorije koja se dinamicki zauzima.
//		Trenutno se zauzima 4 niza dimenzija M*N !!! Dva niza za test
//		i dva niza za podatke koji se koriste za OpenCL program.
#define MATRIX_SIZE 5
#define WORKGROUP_M 4
#define WORKGROUP_N 4
// Dimenzije M i N moraju uvek da budu deljive sa WORKGROUP_M i WORKGROUP_N, respektivno.
#define M (MATRIX_SIZE * WORKGROUP_M)
#define N (MATRIX_SIZE * WORKGROUP_N)
...
int main(int argc, const char** argv)
{
	...
	//TODO: allocate memory for all buffers
	...
	// 13. Define work-item and work-group
		...
		//TODO: Define dimension, global and local work size

	size_t n_dim = 2;
	size_t global_work_size[2] = { M, N };
	size_t local_work_size[2] = { WORKGROUP_M , WORKGROUP_N }; //1, 2, 4, 8 , 16 ,32
	...
	int* test_buffer_C = (int*)malloc(sizeof(int) * data_size);
	...
	print_matrix(host_buffer_C, M, N, "host_buffer_C_after_kernel");

	matrix_multiplication_serial(test_buffer_A, test_buffer_B, test_buffer_C, M, N);
	//print_matrix(test_buffer_C, M, N, "test_buffer_C_serial");
	print_matrix(test_buffer_C, M, N, "host_buffer_C_serial");

	if (!check_results(test_buffer_C, host_buffer_C, M, N)) {
