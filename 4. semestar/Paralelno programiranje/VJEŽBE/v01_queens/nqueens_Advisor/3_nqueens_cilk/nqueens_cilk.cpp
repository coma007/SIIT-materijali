...
cilk::reducer_opadd<int> nrOfSolutions;  //keeps track of the number of solutions
...
void solve() {
    ...
    cilk_for(int i=0; i<boardSize; i++) {
...
int main(int argc, char* argv[]) {
    if(argc !=2) {
        cerr << "Usage: nqueens_cilk boardSize [default is 13].\n";
        boardSize = 13;
    } else {
    ...
    cout << "Starting nqueens (3_nqueens_cilk) solver for size " << boardSize << "...\n";
    DWORD startTime=timeGetTime();
    solve();
    DWORD endTime=timeGetTime();
    cout << "Number of solutions: " << nrOfSolutions.get_value() << endl;
    if (nrOfSolutions.get_value() != correctSolution[boardSize])
